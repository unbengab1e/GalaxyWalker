from transformers.models.qwen2_vl.modeling_qwen2_vl import Qwen2VLPreTrainedModel, GenerationMixin, Qwen2VisionTransformerPretrainedModel, Qwen2VLModel, Qwen2VLCausalLMOutputWithPast,_prepare_4d_causal_attention_mask_with_cache_position
...
@dataclass
class AstroQwen2VLOutputWithPast(ModelOutput):
    loss: Optional[torch.FloatTensor] = None
    logits: torch.FloatTensor = None
    num: torch.FloatTensor = None
...
class AstroQwen2VL(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.num_head = nn.Sequential(
            ...
            nn.GELU(),
            nn.Linear(4*config.hidden_size, 1, bias=True))
    ...
    def forward(
        ...
        video_grid_thw: Optional[torch.LongTensor] = None,
        rope_deltas: Optional[torch.LongTensor] = None,
        answers: Optional[torch.FloatTensor] = None,
        num_token_mask = None
        ...
        number_logits = self.num_head(hidden_states[num_token_mask])
        ...
        if labels is not None:
            ...
            # predict number logits, for simple, just use the last token
            # number_logits = number_logits[..., -1, :].contiguous()
            # import pudb;pu.db;
            reshape_answers = answers.reshape_as(number_logits).float()
            num_loss = torch.nn.functional.mse_loss(number_logits, reshape_answers, reduction="mean")
            loss = loss + num_loss
        ...
        return AstroQwen2VLOutputWithPast(
            loss=loss,
            logits=logits,
            num = number_logits,
...
class AstroQwen2VLSpec(Qwen2VLPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
...
class AstroQwen2VLFull(Qwen2VLPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
    ...
    def __init__(self, config):
        ...
        self.spec_projector = nn.Linear(1024, config.hidden_size, bias=False)
        self.struc_projector = nn.Linear(256, config.hidden_size, bias=False)
        self.model = Qwen2VLModel(config)
        self.vocab_size = config.vocab_size
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
        self.padding_side = "left"  # set it to left by default, user can use setter to change padding_sides
